...
def get_rate_limit_key(request: Request) -> str:
    """Combined key: IP + user_id (if authenticated) for per-user rate limiting."""
...
# For cookie-based auth, we need CSRF protection on state-changing endpoints
# We'll use a double-submit cookie pattern with a custom header
...
async def csrf_protection(request: Request, call_next):
    """CSRF protection for cookie-based auth on state-changing methods."""
    # Set CSRF token cookie if not present (for initial page load)
    # NOT HttpOnly so JavaScript can read it for X-CSRF-Token header
    # Secure=False in test environment (HTTP), True in production
    ...
    if "csrf_token" not in request.cookies:
        ...
        response.set_cookie(
            ...
            secure=not os.environ.get('PYTEST_CURRENT_TEST'),  # Secure=False in tests
            samesite="strict",
            path="/",
    ...
    # Skip CSRF for paths that don't use cookie auth (e.g., webhooks)
    # For now, protect all state-changing API routes
...
async def origin_validation(request: Request, call_next):
    ...
    if request.method in ("POST", "PUT", "DELETE", "PATCH"):
        ...
        # Allow same-origin requests (no origin header) or validate against allowed origins
        allowed_origins = os.environ.get('CORS_ORIGINS', 'http://localhost:3000').split(',')
        
        if origin and origin not in allowed_origins:
            # Check if referer is allowed as fallback
            if referer:
                referer_origin = referer.split('/')[0] + '//' + referer.split('/')[2]
                ...
                return JSONResponse(
                    status_code=403,
                    content={"detail": "Missing origin/referer header"}
                )
...
# Register V1 routers
from app.routers.register import register_v1_routers
register_v1_routers(app, db)

# Health endpoint for testing
@app.get("/api/health")
async def health_check():
    return {"status": "ok", "service": "dollarflow-api"}

# CORS Configuration - no wildcard in production
cors_origins = os.environ.get('CORS_ORIGINS', 'http://localhost:3000')
if cors_origins == '*':
    raise ValueError("CORS_ORIGINS cannot be wildcard '*' in production. Set explicit origins.")
...
async def shutdown_db_client():
    client.close()
    from app.dependencies import close_mongo_client
