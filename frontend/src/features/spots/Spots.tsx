...
import { MapPin, Star, Clock, Search, DollarSign } from 'lucide-react';
import { Input } from '@/shared/ui/input';
import { Badge } from '@/shared/ui/badge';
import { spotsApi } from '@/shared/services/api';
...
function SpotCard({ spot, selected, onClick }: { spot: any; selected: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
    ...
    >
      <div className="flex items-start justify-between mb-2">
        ...
        <Badge variant={spot.is_open ? 'success' : 'danger'} className="text-xs">
          {spot.is_open ? 'Open' : 'Closed'}
        </Badge>
      </div>
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <span className="flex items-center gap-1"><Star className="w-3 h-3 fill-warning text-warning" /> {spot.rating}</span>
        <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {spot.hours}</span>
        <span className="flex items-center gap-1"><DollarSign className="w-3 h-3" /> 1 USDC = {spot.exchange_rate} {spot.currencies[0]}</span>
      </div>
    </button>
  );
}
...
export function Spots() {
  ...
  const fetchSpots = useCallback(async () => {
    try {
      ...
      const res = await spotsApi.list(params);
      setSpots(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  if (loading) {
    return (
      <div className="h-[calc(100vh-3.5rem)] lg:h-screen flex flex-col lg:flex-row">
        <div className="flex-1 relative min-h-[300px] lg:min-h-0 animate-pulse bg-surface-elevated" />
        <div className="w-full lg:w-96 border-t lg:border-t-0 lg:border-l border-border bg-background animate-pulse" />
      </div>
    );
  }
      ...
      <div className="flex-1 relative min-h-[300px] lg:min-h-0">
        ...
        <div className="absolute top-4 left-4 right-4 z-[500] flex gap-2 flex-wrap">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-xs font-medium backdrop-blur-md transition-all ${
              showOpenOnly ? 'bg-success text-white' : 'bg-black/50 text-white/80 hover:bg-black/70'
            }`}
          >
            Open Now
          </button>
      ...
      <div className="w-full lg:w-96 border-t lg:border-t-0 lg:border-l border-border bg-background overflow-y-auto max-h-[40vh] lg:max-h-full">
        <div className="p-4 border-b border-border sticky top-0 bg-background z-10">
          ...
          <div className="relative">
            ...
            <Input placeholder="Search agents..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} data-testid="spots-search" className="pl-9 rounded-xl" />
          </div>
