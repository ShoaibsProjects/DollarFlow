...
export function LandingPage() {
  ...
  useEffect(() => {
    const fetchLiveFees = async () => {
      try {
        const res = await analyticsApi.getLiveFees(200);
        const services = res.data.services;
        const mapped = services.map((s: { name: string; fee: number; speed: string; rate_quality: string; live: boolean }) => ({
          service: s.name,
          fee: `$${s.fee.toFixed(2)}`,
          time: s.speed,
          rate: s.rate_quality,
        ...
        const df = services.find((s: { name: string; fee: number }) => s.name === 'DollarFlow');
        if (df) setLiveDollarFlowFee(`$${df.fee.toFixed(2)}`);
        setHasLiveData(services.some((s: { live: boolean }) => s.live));
      } catch {
        // keep fallback
      }
    };
  ...
  return (
    <div className="min-h-screen bg-background overflow-hidden" data-testid="landing-page">
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-border/50 bg-background/80 backdrop-blur-xl" data-testid="landing-navbar">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-display font-bold text-xl text-foreground">DollarFlow</span>
          </div>
          ...
          <Button
            ...
            onClick={login}
            className="bg-primary hover:bg-primary-hover text-white rounded-full px-6 shadow-[var(--shadow-glow)]"
          >
            Get Started
          </Button>
        </div>
      ...
      {/* Hero Section */}
      <section ref={heroRef} className="relative pt-32 pb-20 md:pt-44 md:pb-32 mesh-gradient" data-testid="hero-section">
        <motion.div style={{ y: heroY }} className="max-w-7xl mx-auto px-4 sm:px-8">
          <div className="max-w-3xl">
            ...
            <motion.h1 {...fadeUp} transition={{ duration: 0.7, delay: 0.1 }}
              className="font-display text-4xl sm:text-5xl lg:text-7xl font-bold text-foreground leading-[1.05] tracking-tight mb-6"
            >
              Your dollars.{' '}
            ...
            <motion.div {...fadeUp} transition={{ duration: 0.7, delay: 0.3 }} className="flex flex-col sm:flex-row gap-4">
              <Button
                ...
                onClick={login}
                size="lg"
                className="bg-primary hover:bg-primary-hover text-white rounded-full px-8 py-6 text-base font-semibold shadow-[var(--shadow-glow)] hover:shadow-[var(--shadow-glow-lg)] transition-all"
              >
              ...
              <Button
                variant="secondary"
                size="lg"
                className="rounded-full px-8 py-6 text-base border-border/50 hover:bg-surface-elevated"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, i) => (
              <motion.div
                ...
                className="group p-6 rounded-2xl border border-border/50 bg-surface hover:border-primary/30 transition-all feature-card"
      ...
      {/* Fee Comparison */}
      <section id="pricing" className="py-20 md:py-32 bg-surface/50" data-testid="pricing-section">
        <div className="max-w-4xl mx-auto px-4 sm:px-8">
          <motion.div {...fadeUp} className="text-center mb-12">
            ...
            <p className="text-base text-muted-foreground">
              ...
              {hasLiveData && (
                <span className="inline-flex items-center gap-1 ml-2 px-2 py-0.5 rounded-full bg-success/10 text-success text-xs font-medium">
          ...
          <motion.div {...fadeUp} className="rounded-2xl border border-border overflow-hidden">
            <div className="grid grid-cols-4 gap-0 p-4 text-sm font-medium text-muted-foreground border-b border-border bg-surface-elevated">
              <div>Service</div>
              <div>Fee</div>
              <div>Speed</div>
              <div>Rate</div>
            ...
            {feeComparison.map((row, i) => (
              ...
              >
                <div className={`font-medium flex items-center gap-1.5 ${row.highlight ? 'text-primary' : 'text-foreground'}`}>
                  {row.service}
                  {row.live && <span className="w-1.5 h-1.5 rounded-full bg-success animate-pulse" />}
                </div>
                <div className={row.highlight ? 'text-success font-semibold' : 'text-foreground'}>{row.fee}</div>
                <div className={row.highlight ? 'text-success font-semibold' : 'text-foreground'}>{row.time}</div>
                <div className={row.highlight ? 'text-success font-semibold' : 'text-foreground'}>{row.rate}</div>
              </div>
            ))}
          </motion.div>
        </div>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-8 text-center">
          <motion.div {...fadeUp}>
            <h2 className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-foreground mb-6">
              ...
              <span className="text-success">{liveDollarFlowFee}</span>
            </h2>
            ...
            <Button
              ...
              onClick={login}
              size="lg"
              className="bg-primary hover:bg-primary-hover text-white rounded-full px-10 py-6 text-base font-semibold shadow-[var(--shadow-glow)]"
            >
              Get Started — It's Free <ArrowRight className="ml-2 w-5 h-5" />
            </Button>
          </motion.div>
